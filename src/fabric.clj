...
#_(defn search [request]
  (container {:mw 6}
    (when (some? (:errors request))
     (errors (:errors request)))

    (coast/form-for ::create-query

      (label {:for "fabric/yards"} "yards")
      ; improve by indicating = or > than x yards, or use range (might want to change build yards to range also)
      (input {:type "number" :name "fabric/yards" :min "0" :max "100" :step ".25"})
...
#_(defn create-query [request]
  (let [[_ errors] (-> #_(coast/validate (:params request) [[:required [:fabric/image :fabric/item-number :fabric/width :fabric/yards :fabric/structure :fabric/shade :fabric/content :fabric/color :fabric/weight]]])
                       ; coast/validate is "Validate the map `m` with a vector of rules `validations`,or a validation fn, such as that created with verily's `combine`"

                       (select-keys [:fabric/image :fabric/item-number :fabric/width :fabric/yards :fabric/structure :fabric/shade :fabric/content :fabric/color :fabric/weight])
                       (coast/insert)
                       (coast/rescue))]
    (if (nil? errors)
      (coast/redirect-to ::result)
      (search (merge request errors)))))


#_(defn result [request]
  (let [rows (coast/q '[:select *
                        :from fabric
                        :order image desc yards desc structure desc color desc])]

; probably change the above query to match whatever new query is coming in    
